...
`01_批量任务` 支持逐炉填写 `手工铝块kg/t`。该字段只用于现场铝块的实际用量补录，不参与 LP 自动优化；批量导出时会按当前价格方案下 `铝块` 的价格单独计算 `手工铝块成本元/t`，并输出 `自动吨钢成本`、`手工铝块成本元/t`、`总吨钢成本`、`自动合金消耗kg/t`、`手工铝块kg/t`、`总合金消耗kg/t`、`炉次总成本`。空值或 `0` 表示本炉不额外计入手工铝块。为了让该字段可计价，`04_合金成分库` 和 `05_价格表` 需要保留或补充铝块物料及价格；若填写了 `手工铝块kg/t` 但价格方案中找不到铝块价格，模板预检会报错。
...
- `铝块`：按现场单独维护，不参与 LP 自动优化；单源 workbook 回算时固定读取 `1.合金成本!AH` 计入总耗和总成本；批量模板/API 则读取 `01_批量任务!手工铝块kg/t`，导出时按铝块价格单独计入总合金消耗和总吨钢成本。
...
- `Als/Alt` 虽然在模板解析层有固定回收率 `0.15`，但只要 `manual_aluminum=true`，优化器就会直接移除 `Als/Alt` 的成分约束，并禁用铝类变量；批量场景由 `手工铝块kg/t` 单独计入导出总耗和总成本。
