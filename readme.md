...
- 单值目标解释采用本轮拍板后的口径：
  ...
  - 除上述情况外，其余元素单值目标都只生成下限，例如 `Si目标=0.23 -> Si>=0.23`、`Mn目标=1.80 -> Mn>=1.80`。
...
- `Ti`：只在下限侧加一次 `+0.005` 安全余量。例如 `Ti目标=0.040` 时，LP 实际约束是 `Ti>=0.045`。
...
- 按当前澄清后的低 Si 语义重新回算后，正确版单源回算得到：`328` 行里 `328` 行 LP 可行；批准规则下旧 Excel 可行 `293` 行、不可行 `34` 行、输入异常 `1` 行。此前出现的 `21` 行低 Si 不可行，已经通过恢复“`Si<=0.05` 按上限语义”解决。
