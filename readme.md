...
批量模板包含任务、目标成分、转炉终点与回收率、合金成分库、价格表、填写说明，以及可编辑的 `07_工艺规则参数`。上传后先预检结构和数据，再逐任务调用同一个优化核心，最后导出结果。旧模板如果没有 `07_工艺规则参数`，当前仍兼容，默认回落到系统规则。
...
- `C`：按目标值扣 `0.005` 控制上限。例如 `C目标=0.040` 时，LP 实际约束是 `C<=0.035`。
- `Si`：当 `Si目标<=0.040` 时，直接禁用 `硅锰/硅铁` 两个变量。
- 单值目标解释采用本轮拍板后的口径：
  - `C/P/S` 单值目标按上限控制。
  - `Si<=0.050` 时只做低杂质控制，按上限控制，不要求补到该目标值。
  - `Ni/Cu/Mo/Sb<=0.020`、`B<=0.0002` 时，直接视为“不投对应纯合金”，不再生成该元素目标约束。
  - 除上述情况外，其余元素单值目标按等值控制，不叠加隐藏安全余量，例如 `Si目标=0.23 -> Si=0.23`、`Mn目标=1.80 -> Mn=1.80`。
...
- `Ti`：旧上下限/下限输入仍只在下限侧加一次 `+0.005` 安全余量；若来自单值目标并已解析成等值约束，则不再额外叠加 Ti 安全余量，避免把 `Ti=目标值` 改成上下限交叉。
...
- `07_工艺规则参数` 里的 `single_target_si_upper_only_max` 默认是 `0.05`，表示 `Si<=0.05` 时按上限语义处理；`disable_silicon_alloys_si_max` 默认是 `0.04`，表示 `Si<=0.04` 时再额外禁用 `硅锰/硅铁`。这两个阈值是分开的，可分别调整。
...
- 按当前澄清后的低 Si + 等值目标语义重新回算后，正确版单源回算得到：`328` 行里 `328` 行 LP 可行；批准规则下旧 Excel 可行 `86` 行、不可行 `241` 行、输入异常 `1` 行。此前出现的 `21` 行低 Si 不可行，已经通过恢复“`Si<=0.05` 按上限语义”解决；旧 Excel 可行数下降则来自高 Si 和常规单值目标改为 `min=max=目标值` 的严格等值校核。
