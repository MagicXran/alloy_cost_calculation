...
- 单值目标解释采用本轮拍板后的 `A` 方案：
  ...
  - 除上述情况外，其余单值目标都只生成下限，例如 `Si目标=0.23 -> Si>=0.23`、`Mn目标=1.80 -> Mn>=1.80`。
...
- 规则洁净化之后，低 Si 牌号会更“硬”：如果 `Si目标<=0.040` 且终点 `Si=0`，同时又禁用了 `硅锰/硅铁`，那么这类行会直接不可行，而不是再走旧代码里的“低 Si 目标按上限控制”隐藏口径。
...
- 规则洁净化后，当前正确版单源回算得到：`328` 行里 `307` 行 LP 可行、`21` 行不可行；批准规则下旧 Excel 可行 `278` 行、不可行 `49` 行、输入异常 `1` 行。当前 `21` 行 LP 不可行全部与低 Si 牌号在 `Si<=0.04` 且禁硅条件下仍要求 `Si` 下限有关。
