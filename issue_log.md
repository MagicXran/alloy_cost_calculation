...
### 2026-07-01 - 外部价格接口必须用 alloyCode 作为唯一匹配键

- 问题现象：外部接口文档初稿把合金价格接口设计成通过 `alloyName` 字符串匹配系统内部合金，存在同名、别名、空格、简繁或人工改名导致价格错配的风险。
- 原因：合金基础信息已改为系统内部手动维护，外部价格接口只同步价格；若继续用展示名称做主键，接口契约无法保证唯一性。
- 修复办法：`docs/外部接口文档.md` 升级到 v1.6，合金价格接口新增必填 `alloyCode` 并作为唯一匹配主键，`alloyName` 改为可选展示字段；结果推送明细中的 `alloyCode` 同步明确为必填。
- 验证方式：全文检查接口文档，确认价格接口示例和字段表均包含必填 `alloyCode`，且不再把 `alloyName` 描述为匹配主键。
- 防复发要求：所有外部接口只允许用稳定编码做系统间匹配主键，名称字段只能用于展示和人工核对，不能作为唯一关联依据。
